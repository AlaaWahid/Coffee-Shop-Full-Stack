...
from sqlalchemy.sql.elements import IndexExpression
...
def get_drinks_detail(payload):
    ...
    if len(data) == 0:
        abort(404)
    allDrinks = [drink.long() for drink in data]
    return jsonify({
        "success": True,
        "drinks": allDrinks
    }), 200
...
def add_drink(payload):
    ...
    try:
        drink = Drink(title=newTitle, recipe=json.dumps(recipe))
        drink.insert()
        return jsonify({
            "success": True,
            "drinks": drink.long()
...
'''
'''
@TODO implement error handler for 422
